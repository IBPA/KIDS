...
class TruthFinder():
    ...
    """
    @classmethod
    def resolve_inconsistencies(cls, df_data, inconsistencies, answers=None, rho=0.5, gamma=0.3):
        """
        ...
        Returns:
            ...
            df_present_belief_and_source_without_inconsistencies: belief vector and
        ...
        # initialize
        df_present_belief_vector = cls.initialize_belief(df_grouped_data)
        np_present_belief_vector = np.matrix(df_present_belief_vector)
        ...
        np_a_matrix, np_b_matrix = cls.create_matrices(
            df_grouped_data, df_source_size_data, inconsistencies, rho)
        ...
        while delta > THRESHOLD and iteration < MAX_NUM_ITERATIONS:
            ...
            if answers is not None:
                inconsistencies_with_max_belief, df_present_belief_and_source_without_inconsistencies = Sums.find_tuple_with_max_belief(np_present_belief_vector, inconsistencies, df_grouped_data)
        ...
        return inconsistencies_with_max_belief, df_present_belief_and_source_without_inconsistencies, np_present_trustworthiness_vector
    ...
    def initialize_belief(df_grouped_data):
        ...
        """
        return pd.DataFrame(df_grouped_data.apply(lambda x: 0))
    ...
    def modify_source_vector(elements, inconsistencies, rho=0.5):
        ...
        Inputs:
            elements: single element from df_belief_source_matrix
            inconsistencies: Dictionary containing inconsistency_id as key
                and list of inconsistent triples + source as value
    ...
    @staticmethod
    def create_matrices(df_grouped_data, df_source_size_data, inconsistencies, rho=0.5):
        """
        ...
        size_of_sources = np.array([df_source_size_data[source] for source in sources])

        np_a_matrix = (np_belief_source_matrix / size_of_sources).T

        df_belief_source_matrix = pd.DataFrame(
            np_belief_source_matrix, index=df_grouped_data.index, columns=sources)
        np_b_matrix = df_belief_source_matrix.apply(
