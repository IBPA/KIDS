...
class Sums:
    ...
    """
    @classmethod
    def resolve_inconsistencies(cls, df_data, inconsistencies, answers=None):
        """
        ...
        # preprocess
        df_source_size_data = df_data.groupby('Source').size()  # number of triples per each source
        df_grouped_data = df_data.groupby(SPO_LIST)['Source'].apply(set)

        # initialize
        np_present_belief_vector = cls.initialize_belief(df_grouped_data)
        np_past_trustworthiness_vector = None
        np_a_matrix, np_b_matrix = cls.create_matrices(df_grouped_data, df_source_size_data)
    ...
    @staticmethod
    def find_tuple_with_max_belief(np_present_belief_vector, inconsistencies, df_grouped_data):
        """
        ...
        Returns:
            ...
            df_belief_and_source_without_inconsistencies: belief vector and
                df_grouped_data concatenated but without the inconsistencies
        """
        df_present_belief_vector = pd.DataFrame(
            np_present_belief_vector, index=df_grouped_data.index)
        df_present_belief_and_source = pd.concat(
            [df_present_belief_vector, df_grouped_data], axis=1)

        df_belief_and_source_without_inconsistencies = df_present_belief_and_source
        ...
        return inconsistencies_with_max_belief, df_belief_and_source_without_inconsistencies
    ...
    def initialize_belief(df_grouped_data):
        ...
        """
        return np.matrix(df_grouped_data.apply(lambda x: 0.5)).T
