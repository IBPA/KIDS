...
class Investment:
    ...
    """
    @classmethod
    def resolve_inconsistencies(cls, df_data, inconsistencies, answers=None, exponent=1.2):
        """
        Resolve any inconsistency using investment algorithm.

        Inputs:
            df_data: integrated data that needs inconsistencies resolved
        ...
        Returns:
            ...
            df_belief_and_source_without_inconsistencies: belief vector and
                df_grouped_data concatenated but without the inconsistencies
        ...
        # preprocess
        df_source_size_data = df_data.groupby('Source').size()
        df_grouped_data = df_data.groupby(SPO_LIST)['Source'].apply(set)

        # initialize
        np_present_belief_vector = Sums.normalize(
            cls.initialize_belief(df_source_size_data, df_grouped_data, inconsistencies))
        np_past_trustworthiness_vector = cls.initialize_trustworthiness(df_source_size_data)
        np_default_a_matrix, np_b_matrix = cls.create_matrices(df_grouped_data, df_source_size_data)
        ...
        while delta > THRESHOLD and iteration < MAX_NUM_ITERATIONS:
            np_a_matrix = cls.update_a_matrix(
                np_default_a_matrix, np_past_trustworthiness_vector, df_source_size_data)
            ...
            if answers is not None:
                find_tuple_with_max_belief_result = Sums.find_tuple_with_max_belief(
                    np_present_belief_vector, inconsistencies, df_grouped_data)

                inconsistencies_with_max_belief = find_tuple_with_max_belief_result[0]
                df_belief_and_source_without_inconsistencies = find_tuple_with_max_belief_result[1]
        ...
        return inconsistencies_with_max_belief, df_belief_and_source_without_inconsistencies, np_trustworthiness_vector
    ...
    @staticmethod
    def create_matrices(df_grouped_data, df_source_size_data):
        """
        Create matrices to be used to compute the trustworthiness and belief vectors.

        Inputs:
            df_grouped_data: data grouped by 'df_data.groupby(SPO_LIST)['Source'].apply(set)'
                with input shape (a, )
            df_source_size_data: data grouped by 'df_data.groupby('Source').size()'
        ...
        """
        sources = df_source_size_data.index.tolist()
        df_belief_source_matrix = df_grouped_data.apply(Sums.create_source_vector, args=(sources,))
        np_belief_source_matrix = np.matrix(df_belief_source_matrix.tolist())

        np_a_matrix = np_belief_source_matrix.T
        np_b_matrix = np_belief_source_matrix / np.array(df_source_size_data)

        return np_a_matrix, np_b_matrix

    @staticmethod
    def update_a_matrix(np_a_matrix, np_past_trustworthiness_vector, df_source_size_data):
        """
        ...
        """
        np_a_matrix = (np_a_matrix.T / (np.array(df_source_size_data) /
                       np_past_trustworthiness_vector.T)).T

        return np_a_matrix / np_a_matrix.sum(axis=0)

    @staticmethod
    def initialize_belief(df_source_size_data, df_grouped_data, inconsistencies):
        """
        ...
        """
        df_present_belief_vector = df_grouped_data.apply(lambda x: 1)
        ...
        for inconsistent_tuples in inconsistencies.values():
            total_source_size = Investment.get_total_source_size_of_inconsistent_tuples(
                inconsistent_tuples, df_source_size_data)

            for (inconsistent_tuple, sources) in inconsistent_tuples:
                source_size = sum([df_source_size_data[source] for source in sources])
                df_present_belief_vector.loc[inconsistent_tuple] = \
                    float(source_size) / float(total_source_size)

        return np.matrix(df_present_belief_vector).T

    @staticmethod
    def get_total_source_size_of_inconsistent_tuples(inconsistent_tuples, df_source_size_data):
        """
        ...
        for (_, sources) in inconsistent_tuples:
            for source in sources:
                total_source_size = total_source_size + df_source_size_data[source]

        return total_source_size

    @staticmethod
    def initialize_trustworthiness(df_source_size_data):
        """
        ...
        """
        return np.matrix(df_source_size_data.apply(lambda x: 1)).T
